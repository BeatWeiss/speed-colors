...
`);});// SERVER LOGIK (Socket.io)io.on('connection', (socket) => {socket.on('joinRoom', (name) => {if(room.active) return socket.emit('err', 'Das Spiel läuft bereits!');if(room.players.length >= 6) return socket.emit('err', 'Raum voll (max 6 Spieler)!');room.players.push({ id: socket.id, name: name, score: 0 });io.emit('roomUpdate', room);});socket.on('requestStart', () => {if(room.players.length < 2) return socket.emit('err', 'Es werden mindestens 2 Spieler benötigt!');if(room.players[0].id !== socket.id) return socket.emit('err', 'Nur der Host kann starten.');room.active = true;room.winner = null;room.players.forEach(p => p.score = 0);room.log = "Das Spiel beginnt! Gleich geht's los...";io.emit('roomUpdate', room);starteNaechstenBlitz();});socket.on('actionPress', (colorId) => {if (!room.rundenAktiv || room.winner) return;const spieler = room.players.find(p => p.id === socket.id);if (!spieler) return;if (colorId === room.zielFarbe.id) {// RICHTIGE FARBEroom.rundenAktiv = false;spieler.score += 1;room.log = ⚡ ${spieler.name} war am schnellsten! (+1 Punkt);// Erfolgs-Feedback gezielt an diesen einen Client sendensocket.emit('successFeedback');if (spieler.score >= 10) {room.winner = spieler.name;room.log = 👑 ${spieler.name} GEWINNT DAS SPIEL! 👑;io.emit('roomUpdate', room);setTimeout(() => { room.active = false; io.emit('roomUpdate', room); }, 5000);} else {io.emit('roomUpdate', room);starteNaechstenBlitz();}} else {// FALSCHE FARBEspieler.score -= 1;room.log = ❌ ${spieler.name} hat falsch gedrückt! (-1 Punkt);io.emit('roomUpdate', room);}});socket.on('disconnect', () => {const index = room.players.findIndex(p => p.id === socket.id);if(index !== -1) {room.log = ${room.players[index].name} hat das Spiel verlassen.;room.players.splice(index, 1);if(room.players.length < 2) {room.active = false;clearTimeout(rundenTimer);}io.emit('roomUpdate', room);}});});function starteNaechstenBlitz() {room.rundenAktiv = false;room.zielFarbe = null;const verzoegerung = Math.floor(Math.random() * 3000) + 2000;rundenTimer = setTimeout(() => {if (!room.active || room.winner) return;const zufallsIndex = Math.floor(Math.random() * FARBEN.length);room.zielFarbe = FARBEN[zufallsIndex];room.rundenAktiv = true;room.log = "JETZT DRÜCKEN!";io.emit('roomUpdate', room);}, verzoegerung);}
